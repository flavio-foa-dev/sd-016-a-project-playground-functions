...
function splitSentence(Array) {
  let array01 = Array;
  let splitSentence01 = array01.split(/[\s,]+/);
  return splitSentence01;
}
...
function highestCount(numberArray) {
  ...
  for (let indexHC = 0; indexHC < numberArray.length; indexHC += 1) {
    if (higherNumber < numberArray[indexHC]) {
      higherNumber = numberArray[indexHC];
    }
  }
  for (let indexHI = 0; indexHI < numberArray.length; indexHI += 1) {
...
function encode(arrayEncode) {
  ...
  for (let index = 0; index < splitArrayEncode.length; index += 1) {
...
function decode(arrayDecode) {
  ...
  for (let index = 0; index < splitArrayDecode.length; index += 1) {
